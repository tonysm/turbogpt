<li data-controller="chat" id="{{ dom_id($chat) }}" class="relative flex items-center space-x-2 px-4 py-1.5 overflow-hidden rounded-lg bg-neutral-800 text-white text-sm">
    ...
    <form id="{{ dom_id($chat, 'delete_form') }}" data-turbo-permanent action="{{ route('chats.destroy', $chat) }}" method="post" class="z-10" data-turbo-confirm="{{ __('Are you sure you want to delete this chat?') }}" data-action="turbo:submit-end->chat#remove">
        @csrf
        @method('DELETE')
